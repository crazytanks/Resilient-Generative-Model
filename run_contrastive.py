...
import utils
from trainer_constrastive import run_training_and_eval
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="mrqa-few-shot")
    parser.add_argument("--dataset_name", type=str, default="bioasq",
                        choices=['bioasq', 'hotpotqa', 'naturalquestions', 'newsqa ', 'searchqa', 'squad', 'textbookqa',
                                 'triviaqa'])
    parser.add_argument(
        "--gpu_id",
        default=1,
    ...
    model_name = "/remote-home/TCCI33/pretrain_models/bart-large"
    ...
    for train_size in train_sizes:
        ...
        for data_seed in data_seeds:
            ...
            train_srcs, train_trgs = utils.get_data(
                f'{data_dir}/{dataset_name}/{dataset_name}-train-seed-{data_seed}-num-examples-{train_size}.jsonl',
                multi_answer=False)
            _, train_multi_trgs = utils.get_data(
                f'{data_dir}/{dataset_name}/{dataset_name}-train-seed-{data_seed}-num-examples-{train_size}.jsonl',
                multi_answer=True)
            ...
            dev_srcs, dev_trgs = utils.get_data(
                f'{data_dir}/{dataset_name}/{dataset_name}-train-seed-42-num-examples-1024.jsonl', multi_answer=False)
            _, dev_multi_trgs = utils.get_data(
                f'{data_dir}/{dataset_name}/{dataset_name}-train-seed-42-num-examples-1024.jsonl', multi_answer=True)
